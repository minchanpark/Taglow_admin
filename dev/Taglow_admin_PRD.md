...
- 조직/팀/결제/멤버십 관리
- 실시간 스탠바이미 원격 제어
- 클라이언트에서 최고 관리자 계정을 생성하거나 사용자 role을 `ADMIN`으로 승격하는 기능
...
8. 관리자 권한 승인, role 변경, 최고 관리자 생성 UI는 MVP 범위에서 제외한다.
...
- 성공 시 로그인 화면으로 이동하고 완료 메시지를 표시한다.
- 클라이언트에서 `ADMIN` role 생성, 부여, 승격 API를 호출하지 않는다.
