...
| Backend | Spring Boot |
| Auth | Spring session/cookie 기반 ADMIN 로그인 또는 token 방식 |
...
- 로그인했지만 `ADMIN` role이 없으면 “관리자 권한이 필요합니다.” 메시지를 표시하고 관리자 화면 진입을 막는다.
...
| vote 생성 | 현재 Swagger에는 `POST /api/public/votes`만 존재. ADMIN 보호 create endpoint 필요 |
...
- ADMIN 보호 `POST /api/votes`가 생기면 config만 `/api/votes`로 바꾸고 View/Controller는 수정하지 않는다.
...
관리자 서비스는 Spring ADMIN 인증을 이미 사용하므로, post-MVP 또는 보안 강화를 위해 서버 발급 presigned URL 방식을 사용할 수 있다.
...
| 구분 | 조건 |
|---|---|
| Backend | ADMIN 보호 vote 생성 endpoint 필요 |
